...
/*
 * Normal handcrafted life events.
 * These remain part of the game.
 */
const DECK: GameCard[] = [
  {
    kind: "life",
 ...
 *
 * This is NOT presented as live news.
 * It uses a real public source and is clearly marked as cached.
 *
 * This means the game can never freeze just because Wi-Fi,
 * the news API, or NVIDIA is unavailable.
 */
const FALLBACK_NEWS_CARD = {
  kind: "news",
  title: "YOUR RENT RENEWAL JUST GOT MORE EXPENSIVE",
  body:
    "Housing costs are putting more pressure on renters. Your landlord sends a renewal that raises your rent by $150 per month.",
  options: [
    {
    ...
    },
    {
      ...
      net: 50,
      debt: 0,
      note: "Moving costs hurt now, but your monthly budget improves.",
    },
  ],
  source: {
    ...
    evidence:
      "Consumer Price Index data tracks changes in prices paid by consumers for goods and services, including shelter.",
...
export default function BrokeBy30() {
  ...
  /*
   * Every third event becomes an Xtract news event.
   */
  const isNewsTurn =
  ...
  /*
   * Keep timeline scrolled to newest entry.
   */
  useEffect(() => {
    if (feedRef.current) {
  ...
  /*
   * AUTOMATIC XTRACT NEWS PIPELINE
   *
   * IMPORTANT:
   * newsState is intentionally NOT a dependency.
   *
   * The old implementation changed newsState inside this effect,
   * which caused React to run the cleanup and abort the request.
   */
  useEffect(() => {
    if (
      !isNewsTurn ||
    ...
    /*
     * Hard timeout.
     * The game will NEVER wait more than 8 seconds.
     */
    ...
    console.log("[XTRACT] Starting automatic news scan");

    /*
     * Give the player visible pipeline progress.
     */
    const stageTimer = window.setTimeout(() => {
      if (active) {
        setNewsState("extracting");
        console.log("[XTRACT] Extracting financial signal");
      }
    }, 600);

    /*
     * Kill network request after 8 seconds.
     */
    const timeoutId = window.setTimeout(() => {
      if (!controller.signal.aborted) {
        ...
        console.warn(
          "[XTRACT] Live request timed out. Switching to cached public source."
        );
    ...
    async function loadNewsEvent() {
      try {
        const response = await fetch("/api/game-news", {
          method: "GET",
          cache: "no-store",
        ...
        if (!body.event) {
          throw new Error(
            body.error ??
              "Xtract did not return a usable event"
          );
        }

        console.log(
          "[XTRACT] Event generated successfully:",
          body.event.title
        );
        ...
        console.log(
          ...
          body.event.source?.extractionMode
        );
         ...
         *
         * the player still receives an Xtract news event.
         */
        setNewsCard(FALLBACK_NEWS_CARD);
        setNewsState("idle");
        ...
        setFeed((items) => [
          ...
          {
            t: requestTimedOut
              ? "LIVE XTRACT TIMED OUT - USING CACHED PUBLIC SOURCE."
              : "LIVE XTRACT UNAVAILABLE - USING CACHED PUBLIC SOURCE.",
            tone: "mute",
          },
        ]);

        console.log(
          "[XTRACT] Cached BLS fallback event loaded"
        );
      } finally {
        ...
        console.log(
          "[XTRACT] News loading cycle finished"
        );
      }
    }
    ...
    return () => {
      /*
       * This cleanup now only occurs when we actually leave
       * this turn / phase or unmount the page.
       *
       * Changing scanning -> extracting no longer aborts
       * the request.
       */
      active = false;
  ...
  const choose = (option: GameOption) => {
    ...
    /*
     * Preserve Xtract provenance in the history.
     */
    if (
      card.kind === "news" &&
      card.source
  ...
  const ageUp = () => {
    ...
    /*
     * Reset Xtract state for next news turn.
     */
    setNewsCard(null);
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          Game scenario amounts are
          simulated for gameplay. The
          source provides the real-world
          financial signal.
        </p>
